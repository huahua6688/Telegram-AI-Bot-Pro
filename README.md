...
### AI 与多模态能力
- 多平台 AI 提供商（OpenAI 兼容 / Anthropic / Gemini）
- 图片输入分析
- 语音转文字后继续对话
- 文本转语音 `/tts`
- 图片生成 `/image`
...
如果你不用 OpenAI 官方，可在 `AI_PROVIDER` 中切换到 OpenAI 兼容 / Anthropic / Gemini（见下方“AI 提供商兼容配置示例”）。
...
| `AI_PROVIDER` | 提供商类型：`openai-compatible` / `anthropic` / `gemini` |
...
3. 在 **Variables** 中填写以下必填项：
   - `BOT_TOKEN` — Telegram Bot Token
   - `AI_PROVIDER` — `openai-compatible` / `anthropic` / `gemini`
   - `AI_MODEL` — 模型 ID
   - `AI_API_KEY`（或对应 `ANTHROPIC_API_KEY` / `GEMINI_API_KEY`）— AI API Key
