...
export class PrivacyTelegramAIBot extends TelegramAIBot {
  ...
  createBottomKeyboard(locale = 'zh') {
    ...
    if (Array.isArray(rows)) {
      rows.splice(1, 0, [this.getPrivacyLabel(locale)]);
    }
    return keyboard;
  }
  ...
  createPrivacyPanelKeyboard(locale = 'zh') {
    return Markup.inlineKeyboard([
      ...
      [Markup.button.callback(localText(locale, '⬅️ 返回设置', '⬅️ Settings'), 'settings_pick:overview')]
    ]);
  }
  ...
  async handleActiveMode(ctx, mode) {
    ...
    if (/^(退出|退出模式|退出隐私|结束|关闭|stop|exit|cancel)$/i.test(input)) {
      this.clearActiveMode(ctx);
      await ctx.reply(localText(locale, '已退出隐私聊天，临时上下文已清除。', 'Private chat exited and temporary context cleared.'), this.createSettingsKeyboard(locale));
      return true;
    }
